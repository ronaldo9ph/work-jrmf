<template lang="html">
  <div class="fundDetail" id="fundDetail">
      <div class="top">
          ...
          <ul class="item clearfix" v-if="fundtype=='货币型'">
              <li>
                  <p>七日年化</p>
                  <p>{{accum_NET}}%</p>
              </li>
      ...
      <div class="box chart">
          <div class="t clearfix">
              <h3 class="title pull-left" v-if="fundtype=='货币型'">七日年化</h3>
              <h3 class="title pull-left" v-else>净值走势</h3>
              <router-link :to="{ name: 'detailhistory', params: {id:fundcode} }" class="arrow pull-right" v-if="fundtype=='货币型'">查看万份收益</router-link>
              <router-link :to="{ name: 'detailhistory', params: {id:fundcode} }" class="arrow pull-right" v-else>查看历史净值</router-link>
          </div>
...
export default {
  ...
  created: async function () {
    ...
    if (res.data.fstat === 1) {
      ...
      this.accum_NET = res.data.fundinfo.accum_NET
